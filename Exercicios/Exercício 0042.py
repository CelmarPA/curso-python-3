...
if (s1 + s2) > s3 and (s1 + s3) > s2 and (s2 + s3) > s1:
    if s1 == s2 == s3:
        print(f'Os segmentos {cores["green"]}PODEM FORMAR{cores["reset"]} um triângulo '
              f'{cores["blue"]}EQUILÁTERO{cores["reset"]}!')
    elif s1 == s2 != s3 or s1 == s3 != s2 or s2 == s3 != s1:
        print(f'Os segmentos {cores["green"]}PODEM FORMAR{cores["reset"]} um triângulo '
              f'{cores["yellow"]}ISÓSCELES{cores["reset"]}!')
    else:
        print(f'Os segmentos {cores["green"]}PODEM FORMAR{cores["reset"]} um triângulo '
              f'{cores["magenta"]}ESCALENO{cores["reset"]}!')
else:
    print(f'Os segmentos {cores["red"]}NÃO PODEM FORMAR{cores["reset"]} um triãngulo!')
